...
|	
| -------------------------------------------------------------------------
| FILENAME - module_affiliate_marketing_text_ads.php
| -------------------------------------------------------------------------     
|
*/

class Module_Affiliate_Marketing_text_Ads extends Admin_Controller {
	
	function __construct()
	{
		parent::__construct();

		$this->load->model('modules/module_affiliate_marketing_text_ads_model', 'text_ads_model');
	...
	{ 
		...
		$data['page_title'] = 'manage_text_ads';
		
		$data['tools'] = $this->text_ads_model->_get_text_ads($this->session->userdata('per_page'), $data['offset'], $data['sort_column'], $data['sort_order']);
		
		$data['totals'] = $this->db_validation_model->_get_count('affiliate_text_ads');
		...
		$data['pagination'] = $this->db_validation_model->_set_pagination($data['uri'], 'affiliate_text_ads', $this->session->userdata('per_page'), 4, $data['sort_order'], $data['sort_column']);
		
		load_admin_tpl('modules', 'tpl_adm_manage_affiliate_marketing_text_ads', $data);	
	}
	...
	function add()
	{
		...
		$data['page_title'] = 'add_text_ad';
			
		if ($this->_check_text_ad() == false)
		{		
			if (!empty($_POST))
			{
			...
			$options = array(	'instance'	=> 'oEdit1',
								'type' => 'content',
								'content' => html_entity_decode($this->validation->text_ad_body),
								'height'	=> '400',
								'width'	=> '100%',
								'editor_type'	=> 'description',
								'textarea'	=>	'text_ad_body',
								'tags'	=> true,
							);
			...
			load_admin_tpl('modules', 'tpl_adm_manage_affiliate_marketing_text_ad', $data);		
		}
		else
		{	
			$id = $this->text_ads_model->_add_text_ad($_POST);		
	...
	function delete()
	{
		...
		if ($this->aff->_delete_tool('text_ad', (int)($this->uri->segment(4)), 'affiliate_text_ads'))
		{
	...
	function edit()
	{
		...
		$data['page_title'] = 'update_text_ad';
		...
		{		
			...
			{
				$m = $this->text_ads_model->_get_text_ad_details((int)$this->uri->segment(4));
		...
		{	
			$id = $this->text_ads_model->_edit_text_ad($this->validation->id, $_POST);		
	...
	function update()
	{
		...
		if ($this->_check_text_ad_options() == false)
		{		
			if (!empty($_POST))
			{
	...
	function sort_order()
	{
		...
		{	
			$this->aff->_change_sort_order('affiliate_text_ads');
	...
	function change_status()
	{
		...
		if ($this->db_validation_model->_change_status_field('affiliate_text_ads', 'id', (int)$this->uri->segment(4), 'status'))
		{
			$this->session->set_flashdata('success', $this->lang->line('system_updated_successfully'));
	...
	*/
	...
	function _check_text_ad_options()
	{
		$rules['module_affiliate_marketing_text_ads_default_text_ad_width'] = 'trim|required|numeric';
		
		$this->validation->set_rules($rules);

		$fields['module_affiliate_marketing_text_ads_default_text_ad_width'] = $this->lang->line('module_affiliate_marketing_text_ads_default_text_ad_width');
	...
	function _check_text_ad()
	{
		$rules['status'] = 'trim|required|integer';
		$rules['name'] = 'trim|required|min_length[5]|max_length[50]';
		$rules['text_ad_title'] = 'trim|required|min_length[5]|max_length[255]';
		$rules['text_ad_body'] = 'trim|required|min_length[20]';
		...
		$fields['name'] = $this->lang->line('text_ad_name');
		$fields['text_ad_title'] = $this->lang->line('text_ad_title');
		$fields['text_ad_body'] = $this->lang->line('text_ad_body');
	...
	function _check_sort_order()
	{
		...
		foreach ($data as $k => $v)
		{
			if (strstr($k, "tool") == true) 
			{
				...
				$fields[$k] = $this->lang->line('text_ad_id'). ' ' .end($opt) ;
			}
		}
