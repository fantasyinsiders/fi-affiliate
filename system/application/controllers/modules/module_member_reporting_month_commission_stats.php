...
|	
| -------------------------------------------------------------------------
| FILENAME - Module_Member_Reporting_Month_Commmission_Stats.php
| -------------------------------------------------------------------------     
| 
*/

class Module_Member_Reporting_Month_Commission_Stats extends Member_Controller {
	
	function __construct()
	{
		parent::__construct();

		$this->load->model('modules/module_member_reporting_month_commission_stats_model', 'modules');
	...
	function generate()
	{
		...
		$data['page_title'] = $this->lang->line('monthly_commission_stats_by_day');
		
		$data['cmonth'] = !(empty($m)) ? $m : date("m");
		$data['cyear'] = !(empty($y)) ? $y : date("Y");
		
		$data['select_months'] = _generate_month_dropdown($data['cmonth'] . '/' . $data['cyear'], 'form-control' , 'document.location=\'' . site_url('members') . '/report/' . $data['module_name'] . '/' . '\' + this.value');
		...
		if (!empty($stats))
		{
			...
			$data['show_graph'] = $this->reports->_generate_js_chart_data($srow, 'stats_graph', 'HighRollerColumnChart', $data['page_title'], $chart_grid_color , false, $chart_bg_color);
