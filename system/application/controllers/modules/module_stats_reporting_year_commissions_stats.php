...
|	
| -------------------------------------------------------------------------
| FILENAME - Module_Stats_Reporting_Year_sales_Stats.php
| -------------------------------------------------------------------------     
| 
...
class Module_Stats_Reporting_Year_Commissions_Stats extends Admin_Controller {
	
	function __construct()
	{
		parent::__construct();
		//load modules model
		$this->load->model('modules/module_stats_reporting_year_commissions_stats_model', 'modules');
		...
		//module name
		$this->config->set_item('module_name', 'module_stats_reporting_year_commissions_stats');
	...
	function generate()
	{
		...
		*/
		...
		//set data array
		$this->config->set_item('num_options', get_default_currency($this->config->item('sts_site_default_currency'), true));
		$data = $this->security_model->_load_config('admin', __CLASS__,  __FUNCTION__);
		
		$data['page_title'] = $this->lang->line('yearly_commissions_stats_by_month');
		...
		//generate the data
		$stats = $this->modules->_generate_report($data['cyear']);
		...
		if ($data['print_page'] == 'archive')
		{
			...
			$name = $data['cyear'] . ' ' . $data['page_title'] . ': ' . date($data['sts_admin_time_format'], _generate_timestamp());
