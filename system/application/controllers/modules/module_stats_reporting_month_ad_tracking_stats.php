...
|	
| -------------------------------------------------------------------------
| FILENAME - Module_Stats_Reporting_Month_Ad_Tracking_Stats.php
| -------------------------------------------------------------------------     
| 
...
class Module_Stats_Reporting_Month_Ad_Tracking_Stats extends Admin_Controller {
	
	function __construct()
	{
		parent::__construct();
		//load modules model
		$this->load->model('modules/module_stats_reporting_month_ad_tracking_stats_model', 'modules');
		...
		//module name
		$this->config->set_item('module_name', 'module_stats_reporting_month_ad_tracking_stats');
	...
	function generate()
	{
		...
		*/
		...
		$data['page_title'] = $tracker['name'] . ' - ' . $this->lang->line('daily_ad_tracking_stats_by_month') . ' (' . $this->lang->line('traffic') . ')';
		...
		//generate the data
		$stats = $this->modules->_generate_report($data['cmonth'], $data['cyear'], (int)$this->uri->segment(4));
	...
	function generate_sales()
	{
		...
		*/
		...
		$data['page_title'] = $tracker['name'] . ' - ' . $this->lang->line('daily_ad_tracking_stats_by_month') . ' (' . $this->lang->line('commissions') . ')';
		$data['row_name'] = 'day_of_month';
		...
		//generate the data
		$stats = $this->modules->_generate_sales_report($data['cmonth'], $data['cyear'], (int)$this->uri->segment(4));
