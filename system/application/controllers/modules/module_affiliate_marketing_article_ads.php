...
|	
| -------------------------------------------------------------------------
| FILENAME - module_affiliate_marketing_article_ads.php
| -------------------------------------------------------------------------     
|
*/

class Module_Affiliate_Marketing_Article_Ads extends Admin_Controller {
	
	function __construct()
	{
		parent::__construct();

		$this->load->model('modules/module_affiliate_marketing_article_ads_model', 'article_ads_model');
	...
	{ 
		...
		$data['page_title'] = 'manage_article_ads';
		
		$data['tools'] = $this->article_ads_model->_get_article_ads($this->session->userdata('per_page'), $data['offset'], $data['sort_column'], $data['sort_order']);
		
		$data['totals'] = $this->db_validation_model->_get_count('affiliate_article_ads');
		...
		$data['pagination'] = $this->db_validation_model->_set_pagination($data['uri'], 'affiliate_article_ads', $this->session->userdata('per_page'), 4, $data['sort_order'], $data['sort_column']);
		
		load_admin_tpl('modules', 'tpl_adm_manage_affiliate_marketing_article_ads', $data);	
	}
	...
	function add()
	{
		...
		$data['page_title'] = 'add_article_ad';
			
		if ($this->_check_article_ad() == false)
		{		
			if (!empty($_POST))
			{
			...
			$options = array(	'instance'	=> 'oEdit1',
								'type' => 'content',
								'content' => html_entity_decode($this->validation->article_ad_body),
								'height'	=> '400',
								'width'	=> '100%',
								'editor_type'	=> 'description',
								'textarea'	=>	'article_ad_body',
								'tags'	=> true,
							);
			...
			load_admin_tpl('modules', 'tpl_adm_manage_affiliate_marketing_article_ad', $data);		
		}
		else
		{	
			$id = $this->article_ads_model->_add_article_ad($_POST);		
	...
	function delete()
	{
		...
		if ($this->aff->_delete_tool('article_ad', (int)($this->uri->segment(4)), 'affiliate_article_ads'))
		{
	...
	function edit()
	{
		...
		$data['page_title'] = 'update_article_ad';
		...
		{		
			...
			{
				$m = $this->article_ads_model->_get_article_ad_details((int)$this->uri->segment(4));
		...
		{	
			$id = $this->article_ads_model->_edit_article_ad($this->validation->id, $_POST);		
	...
	function update()
	{
		...
		if ($this->_check_article_ad_options() == false)
		{		
			if (!empty($_POST))
			{
	...
	function sort_order()
	{
		...
		{	
			$this->aff->_change_sort_order('affiliate_article_ads');
	...
	function change_status()
	{
		...
		if ($this->db_validation_model->_change_status_field('affiliate_article_ads', 'id', (int)$this->uri->segment(4), 'status'))
		{
			$this->session->set_flashdata('success', $this->lang->line('system_updated_successfully'));
	...
	*/
	...
	function _check_article_ad_options()
	{
		return false;
	}
	...
	function _check_article_ad()
	{
		$rules['status'] = 'trim|required|integer';
		$rules['name'] = 'trim|required|min_length[5]|max_length[50]';
		$rules['article_ad_title'] = 'trim|required|min_length[5]|max_length[255]';
		$rules['article_ad_body'] = 'trim|required|min_length[20]';
		...
		$fields['name'] = $this->lang->line('article_ad_name');
		$fields['article_ad_title'] = $this->lang->line('article_ad_title');
		$fields['article_ad_body'] = $this->lang->line('article_ad_body');
	...
	function _check_sort_order()
	{
		...
		foreach ($data as $k => $v)
		{
			if (strstr($k, "tool") == true) 
			{
				...
				$fields[$k] = $this->lang->line('article_ad_id'). ' ' .end($opt) ;
			}
		}
