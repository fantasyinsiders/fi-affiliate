...
<div class="row">
    ...
    <div class="col-md-8 text-right">
		<?php if ($function == 'update_product_commission'): ?>
        <?=_previous_next('previous', 'product_commissions', $this->validation->id);?>
         <a data-href="<?=admin_url()?>product_commissions/delete_product_commission/<?=$this->validation->id?>" data-toggle="modal" data-target="#confirm-delete" href="#" class="md-trigger btn btn-danger"><i class="fa fa-trash-o"></i> <span class="hidden-xs"><?=$this->lang->line('delete')?></span></a>
        <a href="<?=admin_url()?>product_commissions/add_product_commission" class="btn btn-primary"><i class="fa fa-plus"></i> <span class="hidden-xs"><?=$this->lang->line('add')?></span></a>
		<?php endif; ?>
        
        <a href="<?=admin_url()?>product_commissions/view_product_commissions" class="btn btn-primary"><i class="fa fa-search"></i> <span class="hidden-xs"><?=$this->lang->line('view_commissions')?></span></a>
        <?php if ($function == 'update_product_commission'): ?>
        <?=_previous_next('next', 'product_commissions', $this->validation->id);?>
        <?php endif; ?>
    </div>
</div>
<hr />
	...
	<div class="col-md-12">
        ...
        <h4 class="header"><i class="fa fa-cog"></i> <?=$this->lang->line($function)?></h4>
        	<div class="form-group">
        		...
        		<div class="col-lg-5">
        			<input name="product_id" id="product_id" type="text"  value="<?=$this->validation->product_id?>" class="form-control" placeholder="<?=$this->lang->line('product_identifier')?>" />
        		</div>
        	</div>
            <hr />
            <div class="form-group">
            ...
            <div class="form-group">
        		...
        		<div class="col-lg-5">
        			<textarea name="product_description" class="form-control" id="product_description" rows="4"><?=$this->validation->product_description?></textarea>
        		</div>
        	</div>
            <hr />
            <div class="form-group">
