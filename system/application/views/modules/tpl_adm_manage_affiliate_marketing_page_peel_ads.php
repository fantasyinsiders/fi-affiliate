...
<div class="alert alert-warning">
    ...
    <p>
        ...
        <a href="<?=modules_url()?>module_affiliate_marketing_page_peel_ads/add" class="btn btn-warning btn-lg"><?=$this->lang->line('add')?></a>
    </p>
</div>
<?php else: ?>
<form name="prod_form" action="<?=modules_url()?>module_affiliate_marketing_page_peel_ads/sort_order" method="post">
<div class="row">
    <div class="col-md-4">
    <?=_generate_sub_headline('manage_page_peel_ads')?>
    </div>
    <div class="col-md-8 text-right">
    	<?=_previous_next('previous', 'affiliate_page_peel_ads', $pagination['previous'], true);?>
        ...
        <a href="<?=base_url('js')?>modules/module_affiliate_marketing_page_peel_ads/add" class="btn btn-primary"><i class="fa fa-plus"></i> <span class="hidden-xs"><?=$this->lang->line('add_page_peel_ad')?></span></a>
   		<?=_previous_next('next', 'affiliate_page_peel_ads', $pagination['next'], true);?>	     
    </div>
</div>
<hr />
<div class="row">    
        ...
        <table class="table table-striped table-hover">
            <thead>
                <tr>
                    ...
                    <th style="width:35%"><a href="<?=$sort_header?>/page_peel_ad_title" class="sortable"><?=$this->lang->line('page_peel_ad_name')?></a></th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center">
                    	<a href="<?=modules_url()?>module_affiliate_marketing_page_peel_ads/change_status/<?=$v['id']?>/2/<?=$offset?>/<?=$sort_order?>/<?=$sort_column?>/<?=$where_column?>/<?=$show_where_value?>">
                        <?php if ($v['status'] == '1'): ?>
                    ...
                    <td><a href="<?=modules_url()?>module_affiliate_marketing_page_peel_ads/edit/<?=$v['id']?>"><?=$v['page_peel_ad_name']?></a></td>
                    ...
                    <td>
                    	 <div class="text-right">
                         	 <a data-href="<?=modules_url()?>/module_affiliate_marketing_page_peel_ads/delete/<?=$v['id']?>/2/<?=$offset?>/<?=$sort_order?>/<?=$sort_column?>" data-toggle="modal" data-target="#confirm-delete" href="#" class="md-trigger btn btn-danger"><i class="fa fa-trash-o" title="<?=$this->lang->line('delete')?>"></i></a>
                            <a href="<?=modules_url()?>module_affiliate_marketing_page_peel_ads/edit/<?=$v['id']?>" class="btn btn-default" title="<?=$this->lang->line('edit')?>"><i class="fa fa-edit"></i></a>
                        </div>
                    </td>
                </tr>
                <?php endforeach; ?>
