...
<div class="row">
    ...
    <div class="col-md-8 text-right">
		...
		<?=_previous_next('previous', 'affiliate_viral_pdfs', $this->validation->id);?>
        <a href="<?=modules_url()?>module_affiliate_marketing_viral_pdfs/generate/<?=$this->validation->id?>" class="btn btn-info"><i class="fa fa-search-plus"></i> <?=$this->lang->line('preview')?></a>
         <a data-href="<?=modules_url()?>module_affiliate_marketing_viral_pdfs/delete/<?=$this->validation->id?>" data-toggle="modal" data-target="#confirm-delete" href="#" class="md-trigger btn btn-danger"><i class="fa fa-trash-o"></i> <span class="hidden-xs"><?=$this->lang->line('delete')?></span></a>
        <?php endif; ?>
        <a href="<?=modules_url()?>module_affiliate_marketing_viral_pdfs/view" class="btn btn-primary"><i class="fa fa-search"></i> <span class="hidden-xs"><?=$this->lang->line('view_viral_pdfs')?></span></a>
        <?php if ($function == 'edit'):?>
        <?=_previous_next('next', 'affiliate_viral_pdfs', $this->validation->id);?>
        <?php endif; ?>
    </div>
</div>
<hr />
	...
	<div class="col-md-12">
		<div class="box-info">
        	...
        	<div class="form-group">
    		    <label for="name" class="col-sm-3 control-label"><?=$this->lang->line('viral_pdf_name')?></label>
        		<div class="col-lg-5">
            ...
            <hr />
             <div class="form-group">
    		    <label for="viral_pdf_title" class="col-sm-3 control-label"><?=$this->lang->line('viral_pdf_title')?></label>
        		<div class="col-lg-5">
        			<input name="viral_pdf_title" type="text" class="form-control required" value="<?=$this->validation->viral_pdf_title?>" />
        		</div>
        	</div>
            <hr />
            <div class="form-group">
