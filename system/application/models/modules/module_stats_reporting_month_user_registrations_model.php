...
|	
| -------------------------------------------------------------------------
| FILENAME - Module_Stats_Reporting_Month_User_Registrations_model.php
| -------------------------------------------------------------------------     
| 
...
class Module_Stats_Reporting_Month_User_Registrations_Model extends CI_Model {	
	...
	function _generate_report($cmonth = '', $cyear = '')
	{
		...
		for ($i = 1; $i <=$days; $i++)
		{
			...
			$sql .= '(SELECT COUNT(member_id) 
					  ...
					  AND signup_date <= ' . $end . ') as ' . $month . '_' . $day . ' ';
