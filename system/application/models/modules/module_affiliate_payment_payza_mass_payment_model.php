...
|	
| -------------------------------------------------------------------------
| FILENAME - module_affiliate_payment_payza_mass_payment_model.php
| -------------------------------------------------------------------------     
| 
| This model handles the functions for payza mass payment
|
*/

class Module_Affiliate_Payment_payza_Mass_Payment_Model extends CI_Model {	
	...
	{	
		...
		$config = array(
							'settings_key'	=>	'module_affiliate_payment_payza_mass_payment_use_date_range',
							'settings_value'	=>	'0',
		...
		$config = array(
							'settings_key'	=>	'module_affiliate_payment_payza_mass_payment_start_date',
							'settings_value'	=>	'',
							'settings_module'	=>	'affiliate_payment',
							'settings_type'	=>	'text',
							'settings_group'	=>	$id,
		...
		$config = array(
							'settings_key'	=>	'module_affiliate_payment_payza_mass_payment_end_date',
							'settings_value'	=>	'',
							'settings_module'	=>	'affiliate_payment',
							'settings_type'	=>	'text',
							'settings_group'	=>	$id,
		...
		$config = array(
							'settings_key'	=>	'module_affiliate_payment_payza_mass_payment_exclude_minimum',
							'settings_value'	=>	'0',
							'settings_module'	=>	'affiliate_payment',
							'settings_type'	=>	'dropdown',
							'settings_group'	=>	$id,
		...
		$config = array(
							'settings_key'	=>	'module_affiliate_payment_payza_mass_payment_total_rows',
							'settings_value'	=>	'100',
							'settings_module'	=>	'affiliate_payment',
							'settings_type'	=>	'text',
							'settings_group'	=>	$id,
		...
		$config = array(
							'settings_key'	=>	'module_affiliate_payment_payza_mass_payment_payment_details',
	...
	{	
		...
		foreach($data as $member)
		{
			...
			$file .= $member['payza_id'] . $slash . $member['payment_amount'] . $slash . $member['fname'] . ' ' .$member['lname'] . ' - ' . $member['affiliate_note'] ."\n";
		...
		$name = 'payza_mass_payment_' . date('m-d-Y') . '.txt';
	...
	function _get_affiliate_payments($limit = 25, $offset = 0, $sort_column = '', $sort_order = '', $start_date = '', $end_date = '')
	{
		...
		if (!empty($start_date) && !empty($end_date))
		{
			$sql = 'SELECT DISTINCT member_id as id, 
					(SELECT CONCAT(fname,\' \',lname, \'|\',payment_name,\'|\',payza_id,\'|\',payment_preference_amount) as name FROM ' . $this->db->dbprefix('members') . ' 
