...
|	
| -------------------------------------------------------------------------
| FILENAME - import_model.php
| -------------------------------------------------------------------------     
| 
| This model handles the functions for import_import
|
*/
...
class Import_Model extends CI_Model {
	...
	function _get_import_details($id = '')
	{
		//$this->db->join('settings', 'import_import.import_import_id = settings.settings_group');
		$this->db->where('module_id', $id);
		$this->db->where('module_type', 'data_import');
		$query = $this->db->get('modules');
		...
		if ($query->num_rows() > 0)
		{
			...
			//get any configs
			$this->db->where('settings_module', 'data_import');
	...
	function _get_import($limit = 25, $offset = 0, $sort_column = '', $sort_order = '')
	{
		//get all the admins from db for list view
		if (!$sort_order) $sort_order = $this->config->item('dbs_imp_order');
		if (!$sort_column) $sort_column = $this->config->item('dbs_imp_column');
